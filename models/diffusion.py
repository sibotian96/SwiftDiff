...
class Diffusion:
    def __init__(self, noise_steps=1000,
        ...
        self.ddim_timestep_seq = torch.from_numpy(self.ddim_timestep_seq)
        self.ddim_timestep_prev_seq = torch.from_numpy(self.ddim_timestep_prev_seq)
        self.dct = dct
        self.idct = idct
        self.n_pre = n_pre
    ...
    def clean_motion(self, x, t, pred_noise):
        sqrt_alpha_hat = torch.sqrt(self.alpha_hat[t])[:, None, None]
        sqrt_one_minus_alpha_hat = torch.sqrt(1 - self.alpha_hat[t])[:, None, None]
        return (x - sqrt_one_minus_alpha_hat * pred_noise) / sqrt_alpha_hat

    def sample_timesteps_idx(self, n):
        return torch.randint(low=0, high=self.ddim_timesteps, size=(n,)).long()

    def inpaint_complete(self, x, prev_t, traj_dct, mode_dict):
        """
    ...
    def ddim_solver(self, model, traj_dct, traj_dct_mod, mode_dict, ddim_step_idx, noise):
        """
        ...
        with torch.no_grad():
            t = self.ddim_timestep_seq[ddim_step_idx].to(self.device)
            prev_t = self.ddim_timestep_prev_seq[ddim_step_idx].to(self.device)
            ...
            if self.EnableComplete is True:
                 x_prev = self.inpaint_complete(x_prev, prev_t, traj_dct, mode_dict)
