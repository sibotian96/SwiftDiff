...
def compute_stats(diffusion, multimodal_dict, model, logger, cfg):
    ...
    def get_prediction(data, model_select):
        ...
        x_T = torch.randn((sample_num, cfg.n_pre, 3 * cfg.joint_num)).to(cfg.device)
        denoise_step = (torch.ones(sample_num)*diffusion.ddim_timestep_seq[-1]).long().to(cfg.device)
        pred_noise = model_select(x_T, denoise_step, mod=traj_dct_cond)
        pred_x_0 = diffusion.clean_motion(x_T, denoise_step, pred_noise)
        zero_step = torch.zeros(sample_num).long()
        model_pred = diffusion.inpaint_complete(pred_x_0, zero_step, traj_dct, mode_dict)

        traj_est = torch.matmul(cfg.idct_m_all[:, :cfg.n_pre], model_pred)
    ...
    stats_meter = {x: {y: AverageMeter() for y in ['SwiftDiff']} for x in stats_names}
    ...
    for i in tqdm(range(0, K), position=0):
        ...
        if i == K - 1:  # in last iteration, concatenate all candidate pred
            pred = np.concatenate(pred, axis=0)  # [50, 5187, 125, 48] for h36m
            pred = pred[:, :, cfg.t_his:, :]
            ...
            for j in range(0, num_samples):
                ...
                stats_meter['APD']['SwiftDiff'].update(apd)
                stats_meter['ADE']['SwiftDiff'].update(ade)
                stats_meter['FDE']['SwiftDiff'].update(fde)
                stats_meter['MMADE']['SwiftDiff'].update(mmade)
                stats_meter['MMFDE']['SwiftDiff'].update(mmfde)
                stats_meter['ADE-m']['SwiftDiff'].update(ade_m)
                stats_meter['FDE-m']['SwiftDiff'].update(fde_m)
                stats_meter['MMADE-m']['SwiftDiff'].update(mmade_m)
                stats_meter['MMFDE-m']['SwiftDiff'].update(mmfde_m)
                stats_meter['ADE-w']['SwiftDiff'].update(ade_w)
                stats_meter['FDE-w']['SwiftDiff'].update(fde_w)
                stats_meter['MMADE-w']['SwiftDiff'].update(mmade_w)
                stats_meter['MMFDE-w']['SwiftDiff'].update(mmfde_w)
            for stats in stats_names:
                str_stats = f'{stats}: ' + ' '.join(
                    [f'{x}: {y.avg:.6f}' for x, y in stats_meter[stats].items()]
                )
    ...
    with open(file_latest % cfg.result_dir, 'w') as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=['Metric'] + ['SwiftDiff'])
        writer.writeheader()
        for stats, meter in stats_meter.items():
            new_meter = {x: y.avg for x, y in meter.items()}
            new_meter['SwiftDiff'] = new_meter['SwiftDiff'].cpu().numpy()
    ...
    if os.path.exists(file_stat % cfg.result_dir) is False:
        ...
        df = pd.concat([df2, df1['SwiftDiff']], axis=1, ignore_index=True)
