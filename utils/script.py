...
from models.mlp import MotionMLP
from utils import padding_traj


def create_model(cfg):
    """
    create the MLP-based SwiftDiff student model
    """
    model = MotionMLP(
...
def dataset_split(cfg):
    ...
    dataset_dict has a 'test' key used for visualization;
    dataset_multi_test is used to create multi-modal data for metrics.
    """
    if cfg.dataset != 'h36m':
        raise NotImplementedError(f"dataset_split only supports h36m (got {cfg.dataset})")

    dataset_test = DatasetH36M('test', cfg.t_his, cfg.t_pred, actions='all')
    dataset_multi_test = DatasetH36M_multi('test', cfg.t_his, cfg.t_pred,
                                           multimodal_path=cfg.multimodal_path,
                                           data_candi_path=cfg.data_candi_path)

    return {'test': dataset_test}, dataset_multi_test
...
def sample_preprocessing(traj, cfg, mode):
    """
    Preprocess traj for sample_ddim().
    Returns a mode dict, traj_dct, and traj_dct_mod.
    """
    if mode == 'pred':
        n = cfg.vis_col
