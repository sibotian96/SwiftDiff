from torch import tensor
from utils import *
from utils.script import sample_preprocessing


def pose_generator(data_set, model_select, diffusion, cfg, mode=None,
                   action=None, nrow=1):
    """
    stack k rows examples in one gif

    The logic of 'draw_order_indicator' is to cheat the render_animation(),
    because this render function only identify the first two as context and gt, which is a bit tricky to modify.
    """
    traj_np = None
    j = None
    while True:
        poses = {}
        draw_order_indicator = -1
        for k in range(0, nrow):
            if mode == 'pred':
                data = data_set.sample_iter_action(action, cfg.dataset)
            else:
                raise NotImplementedError(f"unknown pose generator mode: {mode}")

            # gt
            gt = data[0].copy()
            gt[:, :1, :] = 0
            data[:, :, :1, :] = 0

            if mode == 'pred':
                if draw_order_indicator == -1:
                    poses['context'] = gt
                    poses['gt'] = gt
                else:
                    poses[f'TransFusion_{draw_order_indicator + 1}'] = gt
                    poses[f'TransFusion_{draw_order_indicator + 2}'] = gt
                gt = np.expand_dims(gt, axis=0)
                traj_np = gt[..., 1:, :].reshape([gt.shape[0], cfg.t_his + cfg.t_pred, -1])

            traj = tensor(traj_np, device=cfg.device, dtype=cfg.dtype)

            mode_dict, traj_dct, traj_dct_mod = sample_preprocessing(traj, cfg, mode=mode)
            sample_num = mode_dict['sample_num']
            x_T = torch.randn((sample_num, cfg.n_pre, 3 * cfg.joint_num)).to(cfg.device)
            denoise_step = (torch.ones(sample_num)*diffusion.ddim_timestep_seq[-1]).long().to(cfg.device)
            pred_noise = model_select(x_T, denoise_step, mod=traj_dct_mod)
            pred_x_0 = diffusion.clean_motion(x_T, denoise_step, pred_noise)
            zero_step = torch.zeros(sample_num).long()
            model_pred = diffusion.inpaint_complete(pred_x_0, zero_step, traj_dct, mode_dict)
            
            #multi
            #denoise_step = (torch.ones(sample_num)*diffusion.ddim_timestep_seq[9]).long().to(cfg.device)
            #x_T, _ = diffusion.noise_motion(model_pred, denoise_step)
            #pred_noise = model_select(x_T, denoise_step, mod=traj_dct_mod)
            #pred_x_0 = diffusion.clean_motion(x_T, denoise_step, pred_noise)
            #model_pred = diffusion.inpaint_complete(pred_x_0, zero_step, traj_dct, mode_dict)

            traj_est = torch.matmul(cfg.idct_m_all[:, :cfg.n_pre], model_pred)
            traj_est = traj_est.detach().cpu().numpy()
            traj_est = post_process(traj_est, cfg)

            if k == 0:
                for j in range(traj_est.shape[0]):
                    poses[f'TransFusion_{j}'] = traj_est[j]
            else:
                for j in range(traj_est.shape[0]):
                    poses[f'TransFusion_{j + draw_order_indicator + 2 + 1}'] = traj_est[j]

            if draw_order_indicator == -1:
                draw_order_indicator = j
            else:
                draw_order_indicator = j + draw_order_indicator + 2 + 1

        yield poses
